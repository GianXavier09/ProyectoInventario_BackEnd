...
public class BrandController {
	...
	@GetMapping
	public ResponseEntity<List<Brand>>
			getAllBrands(){
	...
	public ResponseEntity<Brand>
			getBrandById(@PathVariable Integer id)
			throws Exception
			{
				Brand brand = brandService
						.getBrandById(id).orElseThrow(
								() -> new Exception(
										"La marca no existe"));
	...
	public ResponseEntity<Brand> updateBrand(
			...
			@RequestBody Brand brand)
				throws Exception
				{
					Brand currentBrand = brandService
							.getBrandById(id).orElseThrow(
									() -> new Exception(
											"La marca no existe"));
					currentBrand.setMarcanombre(
							brand.getMarcanombre());
					currentBrand.setEmail(
							brand.getEmail());
					currentBrand.setHomepage(
							brand.getHomepage());
					return new ResponseEntity<Brand>(
							brandService.saveBrad(currentBrand),
							HttpStatus.OK);
				}
